...
class PrestaShopAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._customer_id = os.environ.get("PRESTASHOP_DEFAULT_CUSTOMER_ID")
        self._address_id = os.environ.get("PRESTASHOP_DEFAULT_ADDRESS_ID")
        self._carrier_id = os.environ.get("PRESTASHOP_DEFAULT_CARRIER_ID")
        self._currency_id = os.environ.get("PRESTASHOP_DEFAULT_CURRENCY_ID", "1")
        self._order_state_id = os.environ.get("PRESTASHOP_DEFAULT_ORDER_STATE_ID", "1")
        self._payment_module = os.environ.get("PRESTASHOP_PAYMENT_MODULE", "ps_wirepayment")
        self._payment_label = os.environ.get("PRESTASHOP_PAYMENT_LABEL", "Bank wire")
    ...
    def _find_cart_rule(self, code: str) -> Optional[dict]:
    ...
    def _within_date_range(rule: dict) -> bool:
        ...
        for key, is_start in (("date_from", True), ("date_to", False)):
            ...
            if not raw:
                continue
            try:
                bound = datetime.strptime(raw, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
            except ValueError:
                continue
